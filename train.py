...
parser.add_argument('--data_dir', type=str, default='dataset/tacred')#_example
...
parser.add_argument('--num_layers', type=int, default=2, help='Num of blocks.')
parser.add_argument('--input_dropout', type=float, default=0.5, help='Input dropout rate.')
parser.add_argument('--gcn_dropout', type=float, default=0.5, help='layer dropout rate.')
...
parser.add_argument('--fp16', dest='fp16', action='store_true', help='Lowercase all words.')
...
parser.add_argument('--save_epoch', type=int, default=101, help='Save model checkpoints every k epochs.')
...
def get_start_epoch():
    log_file = model_save_dir + '/' + opt['log']
    ...
    if len(data) <= 1:
        return 1
    if data[-1].startswith('new'):
        last_data = data[-2]
    else:
        last_data = data[-1]
    epoch = int(last_data.split('\t')[0]) + 1
    current_lr = float(last_data.split('\t')[1])
    best_dev_score = float(last_data.split('\t')[-1])

    return epoch, current_lr, best_dev_score
print('cuda count:', torch.cuda.device_count() )
# model
if not opt['load']:
    start_epoch = 1
    file_logger = helper.FileLogger(model_save_dir + '/' + opt['log'], header="# epoch\tlearning_rate\ttrain_loss\tdev_loss\tdev_score\tbest_dev_score")
    trainer = (GCNTrainer(opt, emb_matrix=emb_matrix))
    dev_score_history = []
    current_lr = opt['lr']
else:
    # load pretrained model
    file_logger = helper.FileLogger(model_save_dir + '/' + opt['log'], load = True)
    ...
    start_epoch, current_lr,  best_dev_score = get_start_epoch()
    dev_score_history = [best_dev_score]

id2label = dict([(v,k) for k,v in label2id.items()])


global_step =  (start_epoch - 1) * len(train_batch)
...
for epoch in range(start_epoch, opt['num_epoch']+1):
    ...
    dev_score = dev_f1
    file_logger.log("{}\t{:.7f}\t{:.6f}\t{:.6f}\t{:.4f}\t{:.4f}".format(epoch, current_lr, train_loss,  dev_loss, dev_score, max([dev_score] + dev_score_history)))
